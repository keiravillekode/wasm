(module
  ...
  (param $goal i32) (param $starterBucket i32) (result i32 i32 i32)
    ...
    (loop $move
      ...
      (if (i32.eq (local.get $levelTwo) (local.get $bucketTwo)) (then
        (local.set $levelTwo (i32.const 0)))
        (else (if (i32.eqz (local.get $levelOne)) (then
          (local.set $levelOne (local.get $bucketOne)))
        (else (if (i32.eq (local.get $bucketTwo) (local.get $goal)) (then
          (local.set $levelTwo (local.get $goal)))
        (else
      ...
      (if (i32.eq (local.get $levelOne) (local.get $goal)) (then
        (return (local.get $moves) (local.get $starterBucket) (local.get $levelTwo))))
      (if (i32.eq (local.get $levelTwo) (local.get $goal)) (then (return
        (local.get $moves)
        (i32.sub (i32.const 3) (local.get $starterBucket))
        (local.get $levelOne))))
      (if (i32.gt_u (local.get $moves) (i32.const 1000)) (then
        (return (i32.const -1) (local.get $levelOne) (local.get $levelTwo))))
    (br $move))
    (unreachable)
  )
)
